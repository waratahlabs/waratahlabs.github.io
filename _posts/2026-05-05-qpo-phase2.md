...
The Phase 2 kill condition was explicit: if the git commit win rate stays flat, the pre-filter wasn't the bottleneck. Git commit went 0 wins, 1 classical win, 2 ties. That's flat.
...
Everything else held constant: 24 qubits, `circuit_depth=4`, same 10 goals, 10 reps per goal.
...
**The exception is YAML CI config: 1 win, 3 losses, mean Δ −0.013.** Part A diagonal gave YAML its highest win in that batch (+0.033). Part B off-diagonal turns it negative. YAML is a highly constrained structured output task with a narrow answer space. The co-occurrence cross-terms introduce noise into a landscape that was already well-characterised by individual feature scores. The pattern holds across both phases: QAOA adds value where the answer space is open and feature combinations are non-obvious. It loses ground where the answer space is narrow and greedy already finds the ceiling.
...
Either answer is worth publishing. Neither has been run in this problem class.
